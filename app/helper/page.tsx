...
const IDEA_STARTERS = [
  { id: 'trending', label: 'What\'s trending right now?', icon: '📈' },
  { id: 'underserved', label: 'Find underserved niches', icon: '🔍' },
  { id: 'mashup', label: 'Combine two popular genres', icon: '🔀' },
  { id: 'improve', label: 'Improve an existing game type', icon: '⬆️' },
  { id: 'theme', label: 'Suggest a theme for my template', icon: '🎨' },
  { id: 'random', label: 'Generate random idea', icon: '🎲' },
]
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">Get AI-assisted help brainstorming your next Roblox game</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {IDEA_STARTERS.map((starter) => (
          ...
          >
            ...
            <p className="font-medium mt-2">{starter.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="bg-[#1a1a1a] border border-gray-800 rounded-xl p-6">
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 bg-[#111] border border-gray-700 rounded-lg px-4 py-3 focus:outline-none focus:border-red-500"
          />
          <button
      ...
      {!loading && generatedIdeas.length > 0 && (
        <div className="space-y-6">
          ...
          {generatedIdeas.map((idea, index) => (
            <div key={index} className="bg-[#1a1a1a] border border-gray-800 rounded-xl overflow-hidden">
              {/* Header */}
